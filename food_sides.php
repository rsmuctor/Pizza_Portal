...
<div class="main_content">
  <div class="wrapper">
    ...
    <?php
    // Display the sides
    $sql = "SELECT * FROM food_table WHERE category='sides' AND featured='Yes'";
    // Execute the query
    $res = mysqli_query($conn, $sql);
    // Count the rows
    $count = mysqli_num_rows($res);

    // Check if the sides are available
    if($count > 0) {
      // Sides are available
      ...
      echo "<div class='failed text_center'>❌ Display Sides Failed!<br /><br />⚠️ No Sides Added!</div>";
    }
    ?>
